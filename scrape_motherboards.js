...
async function fetchMotherboardDetails(url) {
  try {
    ...
    specs.querySelectorAll("div.group").forEach((group, index) => {
      ...
      let value =
        group.querySelector("div > p")?.textContent ||
        Array.from(group.querySelectorAll("div > ul > li"))
...
async function scrapePage(pageNumber) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      csvContent += `"${name}","${imageUrl}","${productUrl}","${price}","${
        details.manufacturer || "N/A"
      }","${details.model || "N/A"}","${details.partNumber || "N/A"}","${
        details.socketCPU || "N/A"
      }","${details.formFactor || "N/A"}","${details.chipset || "N/A"}","${
        details.memoryMax || "N/A"
      }","${details.memoryType || "N/A"}","${details.memorySlots || "N/A"}","${
        details.memorySpeed || "N/A"
      }","${details.color || "N/A"}","${details.pcieX16Slots || "N/A"}","${
        details.pcieX8Slots || "N/A"
      }","${details.pcieX4Slots || "N/A"}","${details.pcieX1Slots || "N/A"}","${
        details.pciSlots || "N/A"
      }","${details.m2Slots || "N/A"}","${details.miniPCIeSlots || "N/A"}","${
        details.halfMiniPCIeSlots || "N/A"
      }","${details.miniPCIeMSATASlots || "N/A"}","${
        details.msataSlots || "N/A"
      }","${details.sata6Gb || "N/A"}","${details.onboardEthernet || "N/A"}","${
        details.onboardVideo || "N/A"
      }","${details.usb2Headers || "N/A"}","${
        details.usb2HeadersSinglePort || "N/A"
      }","${details.usb3Gen1Headers || "N/A"}","${
        details.usb3Gen2Headers || "N/A"
      }","${details.usb3Gen2x2Headers || "N/A"}","${
        details.supportsECC || "N/A"
      }","${details.wirelessNetworking || "N/A"}","${
        details.raidSupport || "N/A"
      }","${details.specsNum || "N/A"}"\n`;
    }
