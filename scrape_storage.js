...
if (!fs.existsSync(outputFile)) {
  fs.writeFileSync(
    outputFile,
    "Name,Image URL,Product URL,Price,Manufacturer,Part #,Capacity,Price / GB,Type,Cache,Form Factor,Interface,NVME,Specs Number\n"
  );
}
...
async function scrapePage(pageNumber) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      }","${details.partNumber || "N/A"}","${details.capacity || "N/A"}","${
        details.pricePerGB || "N/A"
      }","${details.type || "N/A"}","${details.cache || "N/A"}","${
        details.formFactor || "N/A"
      }","${details.interface || "N/A"}","${details.nvme ? "Yes" : "No"}","${
