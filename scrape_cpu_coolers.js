...
if (!fs.existsSync(outputFile)) {
  let headers =
    "Name,Image URL,Product URL,Price,Manufacturer,Model,Part #,Fan RPM,Noise Level,Color,Height,CPU Socket,Water Cooled,Fanless,Specs Number\n";
  ...
  try {
    ...
    for (const row of rows) {
      ...
      csvContent += `"${name}","${imageUrl}","${productUrl}","${price}","${
        ...
        details.fanRPM || "N/A"
      }","${details.noiseLevel || "N/A"}","${details.color || "N/A"}","${
        details.height || "N/A"
      }","${details.cpuSocket || "N/A"}","${details.waterCooled || "N/A"}","${
        details.fanless || "N/A"
