...
async function fetchGPUDetails(url) {
  ...
  while (retries > 0) {
    try {
      ...
      console.log(
        `${retries} retries left for ${url}, retrying in 3 seconds...`
      );
  ...
  try {
    ...
    for (const row of rows) {
      ...
      csvContent += `"${name}","${imageUrl}","${productUrl}","${price}","${
        ...
        details.chipset || "N/A"
      }","${details.memory || "N/A"}","${details.memoryType || "N/A"}","${
        details.coreClock || "N/A"
      }","${details.boostClock || "N/A"}","${
        details.effectiveMemoryClock || "N/A"
      }","${details.interface || "N/A"}","${details.color || "N/A"}","${
        details.frameSync || "N/A"
      }","${details.length || "N/A"}","${details.tdp || "N/A"}","${
        details.caseExpansionSlotWidth || "N/A"
      }","${details.totalSlotWidth || "N/A"}","${details.cooling || "N/A"}","${
        details.externalPower || "N/A"
      }","${details.hdmiOutputs || "N/A"}","${
        details.displayPortOutputs || "N/A"
      }","${details.dviDDualLinkOutputs || "N/A"}","${
        details.hdmi21aOutputs || "N/A"
      }","${details.displayPort14Outputs || "N/A"}","${
        details.displayPort14aOutputs || "N/A"
      }","${details.displayPort21Outputs || "N/A"}","${
        details.sliCrossFire || "N/A"
