...
if (!fs.existsSync(outputFile)) {
  let headers =
    "Name,Image URL,Product URL,Price,Manufacturer,Part #,Speed,Form Factor,Modules,Price / GB,Color,First Word Latency,CAS Latency,Voltage,Timing,ECC / Registered,Heat Spreader,Specs Number\n";
...
async function scrapePage(pageNumber) {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      }","${details.partNumber || "N/A"}","${details.speed || "N/A"}","${
        details.formFactor || "N/A"
      }","${details.modules || "N/A"}","${details.pricePerGB || "N/A"}","${
        details.color || "N/A"
      }","${details.firstWordLatency || "N/A"}","${
        details.casLatency || "N/A"
      }","${details.voltage || "N/A"}","${details.timing || "N/A"}","${
        details.eccRegistered || "N/A"
      }","${details.heatSpreader || "N/A"}","${details.specsNum || "N/A"}"\n`;
    }
